...
export const MeetEdit = () => {
    ...
    const isFormInvalid = (!id|| id.trim().length < 5 
    ...
    const moveSelected = (event:any, selected:any) => {
        ...
        if (selected && selected._id && selected.type !== 'wall' 
            && selected.type !== 'floor') {
            const index = objects?.indexOf(selected);

            console.log(selected.type)
            if(selected.type !== 'couch' 
            && selected.type !== 'table' && selected.type !== 'rug'){
                console.log(selected.type, "entrou no if")
                switch(event?.key){
                    case 'ArrowUp':
            ...
            if(selected.type === 'couch' 
            || selected.type === 'table' || selected.type === 'rug' ){
                console.log(selected.type,' entrou no if 2')
                switch(event?.key){
                    case 'ArrowUp':
                        selected.y = selected.y > 1 ? selected.y - 1 : 1
                        break;
